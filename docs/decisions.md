...
**Options considered**:
- A) **Placeholder trampoline**: register a placeholder `EditorDocumentTypeDefinition` for the plugin's declared `uriPattern` at discovery time (before activation), reusing the two existing precedents in the codebase for this exact shape (`ContributionRegistrar.registerPanePlaceholder` + `PlaceholderPaneProvider.tsx` for pending panes; `forceReopenBpmnEditors`/`forceReopenDmnEditors` for the close-and-reopen-to-re-resolve idiom). When opened, the placeholder triggers real activation via `ActivationManager.activatePlugin` (which already handles dedupe and the permission-dialog gate); once `activate()` calls the real `registerWebviewDocumentType()`, the placeholder tab closes and reopens against the now-real registration.
- B) Async pre-activation with no placeholder UI: eagerly activate any plugin with `contributes.editorDocumentTypes` the moment its manifest is discovered (rather than on first file-open), so the real registration always exists before a user could try to open a matching file.

**Decision**: Option A.

**Rationale**: Option B reintroduces exactly the "activate everything at boot" cost that lazy activation exists to avoid — a plugin providing a niche file-type editor would pay full activation cost (including permission dialogs) on every Studio launch regardless of whether the user ever opens such a file, defeating the purpose of declaring it lazily in the first place. Option A activates strictly on demand while still making the file type discoverable (visible in the File Explorer via the same-entry `includedFilePatterns`, openable via double-click) before activation, matching VS Code's `customEditors` UX, and does so by composing two already-proven code paths rather than inventing new activation or dedupe machinery. The manifest contribution deliberately carries no `webviewOptions` — it only promises "something will register this id" — so `activate()`'s `registerWebviewDocumentType()` call remains the single source of truth for actual iframe config, avoiding config drift between the manifest and the real registration. See [architecture/plugin-host.md](architecture/plugin-host.md) §"Static editor document type contributions" for the full mechanism and [common-pitfalls.md](architecture/common-pitfalls.md#declaring-contributeseditordocumenttypes-does-not-by-itself-register-a-webview) for the mismatch/denial failure modes this introduces.
...
### 2026-07-24 — Phase 9: DMN Editor Plugin Enrichment architecture decisions

**Context**: Phase 9 extends the Phase 8 BPMN plugin enrichment model to the DMN editor's DRD (Decision Requirements Diagram) view: overlays, element events, element queries, view awareness, palette/context pad contributions, a modeling API, and renderer module injection, gated by `dmn` / `dmn.modelling` / `dmn.renderer` permissions.

**Decisions**:

1. **DRD-only enrichment scope.** The DMN editor has multiple views (DRD, decision table, literal expression, boxed expression), but only the DRD has a spatial element model that overlays, palette/context pad entries, and modeling operations can meaningfully target. `api.dmn` is scoped exclusively to the DRD: modeling operations throw when the DRD is not active, query operations return empty results, and overlays are cleared/re-resolved on view switches (`EVENT_DMN_ADAPTER_VIEW_CHANGED`).

2. **DMN gets its own `DmnApiBridge` — no generic bridge.** As anticipated in the Phase 8 decision log entry (2026-06-24, item 2), DMN did not receive a shared `EditorEnrichmentBridge`; it received its own explicit `DmnApiBridge`, mirroring `BpmnApiBridge`'s structure and error-message style. The two bridges share the permission-hierarchy mechanism and `PluginHostBridge` namespace-dispatch pattern, but each owns its own element-snapshot building, DRD-gating checks, and modeling validation.

3. **`createElement` (absolute) *and* `appendElement` (relative) — both provided.** DMN's DRD is a loosely-coupled graph of artifacts (decisions, inputs, knowledge sources, BKMs) rather than BPMN's strictly sequential flow. A pure "append relative to source" primitive (BPMN's only creation method) is awkward for bulk DRD layout, while a pure "absolute position" primitive is awkward for the common "insert one related decision" case. Both landed in `api.dmn.modeling`, the SDK `DmnModelingApi` type, the sandbox worker, and the bridge validation, rather than picking one.

4. **No internal `api/DmnApi.ts` class.** The internal `common/plugin-host/api/*` typed-class hierarchy (including `BpmnApi.ts`) was deleted from the codebase on 2026-07-24 (see the preceding decision entry) before Phase 9 began, so there was nothing to parallel. DMN capabilities were added directly to the inline `createPluginApi()` object in `sandbox-worker.ts` (a `dmn: {...}` block alongside the existing `bpmn: {...}` block) and to the SDK contract types in `studio-sdk/src/plugin-api/DmnApi.ts`. `registerOverlayFactory` is present on the SDK `DmnApi` type from the start — the SDK `BpmnApi` had historically omitted it, and Phase 9 deliberately did not repeat that gap.

5. **Duplicated contribution store, palette provider, context pad provider, and module loader** (`PluginDmnContributionStore`, `PluginDmnPaletteProvider`, `PluginDmnContextPadProvider`, `PluginDmnModuleLoader`) rather than generic `<T>`-parameterized shared base classes. The BPMN and DMN versions are structurally near-identical but operate against different diagram-js modeler instances (`BpmnModelerComponentAdapter` vs `DmnModelerComponentAdapter`) and different element-type vocabularies; duplication kept each side simple to read and modify independently, consistent with the plan's "attempt reuse, fall back to duplication" default.

6. **`DmnPluginOverlayManager` is new — DMN had no prior overlay pipeline.** Unlike BPMN (which already had `PluginOverlayStore` wired into an existing overlay concept before Phase 8), DMN had no internal overlay factory pipeline at all before Phase 9. `DmnPluginOverlayManager` was built to combine factory-chain resolution (mirroring `PluginOverlayStore`) with direct rendering of raw DOM overlays onto the DRD's `overlays` diagram-js service, obtained via `DmnModelerComponentAdapter.getDrdOverlays()` / `getDrdElementRegistry()`.

**Rationale**: The DRD-only scope keeps the plugin surface aligned with where DMN actually has a spatial model to operate on, and avoids plugins silently no-op'ing or crashing when a user is looking at a decision table. Providing both element-creation primitives avoids forcing plugin authors into an awkward workaround for whichever primitive was omitted. Duplicating the small per-diagram-type infrastructure classes matches the precedent set by Phase 8's "no generic bridge" decision and keeps each diagram type's plugin integration independently evolvable.

---

### 2026-08-19 — Repair of pre-existing BPMN/DMN plugin API integration test failures: all fixes were test-only, no product change was warranted

**Context**: `plugin-bpmn-modeling.test.ts`, `plugin-bpmn-palette-contextpad.test.ts`, `plugin-bpmn-permissions.test.ts`, `plugin-bpmn-renderer-module.test.ts`, `plugin-dmn-permissions.test.ts`, `plugin-dmn-renderer-module.test.ts`, and `plugin-dmn-api.test.ts` had never passed since the BPMN/DMN plugin API landed (commits `cd9dacce`, `424002ba`). A dedicated plan (`repair_plugin_api_integration_tests_8316792a.plan.md`) grouped the ~34 real failures into five clusters (C1–C5) and investigated each.

**Findings and decisions per cluster**:

1. **C1 (14 tests) — test bug, fixed.** `bpmn:modeler-adapter` is a shared-resource key that was never registered anywhere in `studio/src`; the two affected test files invented it and relied on `Bifrost.getSharedRessource` returning `undefined` on an unknown key, but it actually throws. Fixed by resolving the adapter through the same public API chain the product itself uses (`getEditorDocumentByUri` → `getEditorDocumentModelIfPresent` → `model.modelerAdapter`), installed as a test-only `__bfwResolveBpmnAdapter` global. A secondary defect (task-dependent assertions against a fixture with no tasks) was fixed by switching to a fixture that has them. See `docs/architecture/common-pitfalls.md` for the `getSharedRessource` throw-vs-null-guard pitfall this produced.
2. **C2 (8 tests) — test bug, fixed.** Two files referenced a `simple.bpmn` fixture that was never created and never opened a solution before calling `jumpToFileInSolution`. Fixed by opening the existing fixtures solution and pointing at existing fixture files.
3. **C3b (2 tests) — no product change; already fixed as a side effect of C1/C2.** The plan flagged `modeling.removeElement` and `unregisterContextPadEntry` denial tests as a *possible* privilege-escalation gap requiring explicit approval before any product change. Verified: `PluginHostBridge.executeApiRequest` already gates both correctly under `bpmn.modelling`/`dmn.modelling` (`method.startsWith('modeling.')` and the explicit `unregisterContextPadEntry` check). Both tests pass once run against the C1/C2-fixed suites — the permission gate was never broken; the tests were failing for the same missing-solution/wrong-fixture reasons as C2. **No product change was made or needed.**
4. **C4 (2 tests) — no product change; already passing.** The plan flagged DMN hover/context-menu hit-testing as the least-understood cluster and reserved a product-code investigation. On re-run, `plugin-dmn-api.test.ts` passed 19/19 in isolation, including both flagged tests. **No product change was made or needed** — whatever caused the original failures was resolved as a side effect of unrelated fixture/infra fixes made earlier in the same effort.
5. **C5 (~30 tests under load) — test bug, fixed.** Running the full 12-file directory without `--bail` no longer reproduces the ~30 `beforeAll` timeouts from the original baseline (already resolved as a side effect of the C1–C3a-ii fixes). One new flake surfaced: `plugin-dmn-palette-contextpad.test.ts`'s "palette and context pad lifecycle" `describe` block lacked `{ shuffle: false }`, unlike its already-fixed BPMN counterpart — same order-dependency bug, same fix applied.
