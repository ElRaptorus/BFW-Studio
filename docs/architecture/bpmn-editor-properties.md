...
|------|-----------|----------------|
| `PropertiesCallActivity` | `processModelId` | `PaneBody key={element_process_id_${processModelId}}`. Process `PaneProperty` has no value-based key. Start Event is keyed by `startEventId`. Called process version is a native `PaneProperty type="text"` (`#call-activity-called-process-version-property`), keyed by the current pin string. Empty means unpinned (engine latest at enter). |
