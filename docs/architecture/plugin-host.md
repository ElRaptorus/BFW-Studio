...
| **Settings** | Opens the Settings GUI (`about:settings`). If the plugin registered settings whose `category` (explicit or inferred from the first key segment) matches the plugin's `displayName`, auto-scrolls to that category via `settings.openUserSettingsAtCategory`. Otherwise opens the GUI without scrolling. |
