...
export interface ResolvedContextPadEntry {
  icon: string;
  title: string;
  command: string;
  elementTypes?: string[];
...
/**
 * Singleton store for plugin-contributed palette and context pad entries.
 *
 * Written by ContributionRegistrar (manifest entries) and BpmnApiBridge (runtime entries).
 * Read by PluginPaletteProvider and PluginContextPadProvider (diagram-js modules).
 */
class PluginBpmnContributionStore {
  private paletteEntries = new Map<string, ManifestBpmnPaletteEntry[]>();
  private contextPadEntries = new Map<string, Map<string, ResolvedContextPadEntry>>();
  private listeners: ChangeListener[] = [];

  setPaletteEntries(pluginName: string, entries: ManifestBpmnPaletteEntry[]): void {
  ...
  getAllPaletteEntries(): Map<string, ManifestBpmnPaletteEntry[]> {
    return this.paletteEntries;
  }

  setContextPadEntries(pluginName: string, entries: ManifestBpmnContextPadEntry[]): void {
    const resolved = new Map<string, ResolvedContextPadEntry>();
  ...
  getAllContextPadEntries(): Map<string, Map<string, ResolvedContextPadEntry>> {
  ...
  addPaletteEntry(pluginName: string, entry: ManifestBpmnPaletteEntry): void {
  ...
  private notifyChange(): void {
    for (const listener of this.listeners) {
      try {
        ...
        console.warn('[PluginBpmnContributionStore] Change listener error:', error);
      }
    }
  }
}

export const pluginBpmnContributionStore = new PluginBpmnContributionStore();
