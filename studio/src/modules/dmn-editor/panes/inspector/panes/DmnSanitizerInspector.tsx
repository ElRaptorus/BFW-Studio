import {
  ...
  type DmnSanitizableIssue,
  type DmnSanitizerIssueCategory,
  issueDescriptions,
} from '#modules/dmn-core/sanitizer';
import type { DmnSanitizerBridgeApi } from '#modules/dmn-core/sanitizer/SanitizerBridge';
...
export function DmnSanitizerInspector(props: DmnSanitizerInspectorProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const bridge = model.modelerAdapter.getDmnSanitizerBridge() as DmnSanitizerBridgeApi | null;
    if (!bridge) {
      return;
    }
  ...
  const groupedByCategory = useMemo(() => {
    const groups = new Map<DmnSanitizerIssueCategory, DmnSanitizableIssue[]>();
  ...
  const onFixIssue = useCallback(
    (issue: DmnSanitizableIssue, event: React.MouseEvent) => {
      event.stopPropagation();
      studio.commands.executeCommand('dmn.sanitizer.fixIssue', [issue]);
    },
    [studio],
  );

  const onFixAll = useCallback(() => {
    studio.commands.executeCommand('dmn.sanitizer.fixAll');
  }, [studio]);
    ...
    return (
      <div className="sanitizer-inspector" data-test--sanitizer-inspector>
        <div className="sanitizer-inspector__empty">
          ...
          <span>No issues found. The DMN is clean.</span>
        </div>
      </div>
    );
  }
