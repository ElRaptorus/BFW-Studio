import type { ManifestDmnModule } from '#bifrost/common/plugin-host/manifest/ManifestTypes';
import path from 'path';

import { PluginChannel } from '../../bpmn-core/plugin-modules/PluginChannel';
...
/**
 * Loads plugin-provided diagram-js modules into the DMN DRD renderer process
 * and manages their lifecycle.
 *
 * Each plugin's modules are registered in the DmnModelerModuleRegistry.
...
class PluginDmnModuleLoader {
  ...
  /**
   * Load all declared DMN modules for a plugin.
  ...
  loadPluginModules(
    ...
    try {
      dmnModelerModuleRegistry.registerPluginModule(pluginName, channelModule);

      for (const moduleDeclaration of dmnModules) {
        ...
        dmnModelerModuleRegistry.registerPluginModule(pluginName, resolvedModule);
      }
  ...
  unloadPluginModules(pluginName: string): void {
    ...
    dmnModelerModuleRegistry.unregisterPluginModules(pluginName);
  }
...
export const pluginDmnModuleLoader = new PluginDmnModuleLoader();
