...
const CONFIG: BpmnFragmentRendererConfig = {
  label: 'Condition',
  language: 'feel',
  editorHtmlId: 'bpmn-conditional-event-fragment-editor',
  linkProps: { id: 'bpmn-conditional-event-fragment-link-to-editor-document' },
};

function getFragmentValue(model: BpmnDocumentModel, fragmentId: string): string {
  return model.elements.getElementPropertyValue(fragmentId, 'conditionalEvent').condition;
}

function setFragmentValue(model: BpmnDocumentModel, fragmentId: string, value: string): void {
  model.elements.setElementProperty(fragmentId, 'conditionalEvent', {
    condition: value,
  });
}

/**
 * Used to render the condition property of a BPMN Conditional Event element in a separate editor.
 */
export default function BpmnConditionalEventFragmentRenderer(
