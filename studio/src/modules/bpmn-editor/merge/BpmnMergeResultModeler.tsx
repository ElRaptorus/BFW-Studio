...
import type { ClassifiedElement } from './BpmnMergeResolver';
import { type ResolutionMap, xmlMergeEngine } from './autoApplyEngine';

const CP_KEY_SEPARATOR = ':cp:';

export function makeBaseKey(elementId: string): ConflictKey {
  return elementId;
}

export function makeCpKey(elementId: string, propertyName: string): ConflictKey {
  return `${elementId}${CP_KEY_SEPARATOR}${propertyName}`;
}

export function parseConflictKey(key: ConflictKey): { elementId: string; propertyName: string | null } {
  const idx = key.indexOf(CP_KEY_SEPARATOR);
  if (idx === -1) {
    return { elementId: key, propertyName: null };
  }
  return { elementId: key.substring(0, idx), propertyName: key.substring(idx + CP_KEY_SEPARATOR.length) };
}
...
export type ResultModelerApi = {
  ...
  getCanvas: () => any;
  getOverlays: () => any;
  getViewer: () => BpmnViewerWithSync | null;
};

type BpmnMergeResultModelerProps = {
  studio: any;
  startingSideXml: string;
  applySideXml: string;
  applySideChanges: BpmnDiffChangesByAction | null;
...
type BpmnMergeResultModelerState = {
  loading: boolean;
};

/**
 * Read-only result preview for the merge editor.
 *
 * Uses the XML-level merge engine (`xmlMergeEngine`) both for initial
 ...
 *
 * Resolution is tracked at the **conflict key** level:
 ...
 */
export default class BpmnMergeResultModeler extends React.Component<
  BpmnMergeResultModelerProps,
  BpmnMergeResultModelerState
> {
  private containerRef: React.RefObject<HTMLDivElement | null>;
  private viewer: BpmnViewerWithSync | null = null;
  ...
  constructor(props: BpmnMergeResultModelerProps) {
    super(props);
  ...
  async componentDidUpdate(prevProps: BpmnMergeResultModelerProps): Promise<void> {
  ...
  private async initialize(): Promise<void> {
    ...
    const conflictIds = new Set(
      this.props.classifiedElements.filter((el) => el.classification === 'both').map((el) => el.elementId),
    );

    let mergedXml = this.props.startingSideXml;

    if (this.props.applySideChanges != null) {
      const result = xmlMergeEngine(
    ...
    // --- Visible viewer ---
    this.viewer = new BpmnViewerWithSync();
  ...
  /**
   * For each conflicting element, create compound keys:
   * - One base-property key if it has base attribute changes or actions
   * - One key per custom property that changed on BOTH sides
   */
  private initializeConflictKeys(): void {
    ...
    for (const element of this.props.classifiedElements) {
      ...
      const ours = element.ours;
      const theirs = element.theirs;
      const hasBaseChanges =
        (ours != null && (ours.actions.length > 0 || ours.attributeChanges.length > 0)) ||
        (theirs != null && (theirs.actions.length > 0 || theirs.attributeChanges.length > 0));

      if (hasBaseChanges) {
        this.resolutionMap.set(makeBaseKey(element.elementId), 'pending');
      }

      const oursCpNames = new Set((ours?.customPropertyChanges ?? []).map((cp) => cp.propertyName));
      const theirsCpNames = new Set((theirs?.customPropertyChanges ?? []).map((cp) => cp.propertyName));

      const conflictingCpNames = [...oursCpNames].filter((name) => theirsCpNames.has(name));

      const startSideCpNames = isRebase ? theirsCpNames : oursCpNames;
      const applySideCpNames = isRebase ? oursCpNames : theirsCpNames;

      for (const name of startSideCpNames) {
        if (!applySideCpNames.has(name)) {
          conflictingCpNames.push(name);
        }
      }
      for (const name of applySideCpNames) {
        if (!startSideCpNames.has(name)) {
          conflictingCpNames.push(name);
        }
      }

      for (const cpName of conflictingCpNames) {
        this.resolutionMap.set(makeCpKey(element.elementId, cpName), 'pending');
      }

      if (!hasBaseChanges && conflictingCpNames.length === 0) {
        this.resolutionMap.set(makeBaseKey(element.elementId), 'pending');
      }
    }
  }
  ...
  private computeEffectiveSkipIds(): Set<string> {
    ...
    // Remove conflict elements where ANY sub-key is resolved to apply side
    ...
    // Add reverted auto-applied elements back to the skip set
    for (const elementId of this.initialAutoAppliedIds) {
  ...
  private buildResolvedXml(): string {
    ...
    const { mergedXml } = xmlMergeEngine(
      this.props.startingSideXml,
      this.props.applySideXml,
      this.props.applySideChanges,
      skipIds,
  ...
  private async rebuildAndRefresh(): Promise<void> {
    ...
    try {
      ...
      console.warn('[BpmnMergeResultViewer] Failed to rebuild and refresh:', err);
    }
  }
  ...
  private computeElementOverlayStatuses(): Map<string, ElementResolutionStatus> {
    ...
    for (const [key, status] of this.resolutionMap) {
      const { elementId } = parseConflictKey(key);
  ...
  getConflictKeysForElement(elementId: string): ConflictKey[] {
    ...
    for (const key of this.resolutionMap.keys()) {
      const parsed = parseConflictKey(key);
      if (parsed.elementId === elementId) {
        keys.push(key);
      }
    }
    return keys;
  ...
  // --- Element-level batch resolution (resolves all sub-keys) ---
  ...
  isFullyResolved(): boolean {
    ...
    for (const [key, status] of this.resolutionMap) {
      const { elementId } = parseConflictKey(key);
      if (conflictElementIds.has(elementId) && status === 'pending') {
        return false;
      }
    }
    return true;
  ...
  getAllResolutions(): ElementResolution[] {
    return Array.from(this.resolutionMap.entries()).map(([key, status]) => ({
      key,
      elementId: parseConflictKey(key).elementId,
      status,
    }));
  }
  ...
  render(): React.JSX.Element {
    return <div ref={this.containerRef} className="bpmn-merge__result-container" />;
  }
}
