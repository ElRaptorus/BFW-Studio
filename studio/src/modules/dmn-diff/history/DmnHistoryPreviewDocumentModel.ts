...
import DmnDiffDocumentModel, { type ElementLike } from '../DmnDiffDocumentModel';

export default class DmnHistoryPreviewDocumentModel extends DmnDiffDocumentModel {
  ...
  private previewViewer: DmnViewerWithSync;
  ...
  constructor(uri: string, restoredCurrentData: any, restoredMetadata: any, fileLoader: ILoadable, bifrost: Bifrost) {
    ...
    this.previewViewer = new DmnViewerWithSync();
  }
  ...
  static override async create(
    ...
    bifrost: Bifrost,
  ): Promise<DmnHistoryPreviewDocumentModel> {
    const model = new DmnHistoryPreviewDocumentModel(uri, restoredCurrentData, restoredMetadata, fileLoader, bifrost);
  ...
  protected override async initialize(): Promise<void> {
    ...
    try {
      ...
      if (this.historicalXml == null || this.historicalXml.trim() === '') {
        throw new Error(`Could not load file at ref ${this.commitHash.substring(0, 7)}`);
      }
  ...
  async attachPreview(htmlElement: HTMLDivElement): Promise<void> {
    if (this.historicalXml != null) {
      await this.previewViewer.loadXml(this.historicalXml);
      this.previewViewer.attachTo(htmlElement);
    }
  }
