...
export default function DmnHistoryPreviewDocumentRenderer(
  props: EditorDocumentRendererProps,
): React.JSX.Element | null {
  const { studio: bifrost, editorDocument } = props;

  const [model, setModel] = useState<DmnHistoryPreviewDocumentModel | null>(null);
  ...
  useEffect(() => {
    ...
    bifrost.editors
      .getEditorDocumentModel<DmnHistoryPreviewDocumentModel>(editorDocument)
      .then((loadedModel) => {
        if (cancelled) {
          return;
        }
  ...
  return (
    <Editor>
      <EditorTitle>
        <EditorTitleLeft>
          <EditorTitleHeroIcon studio={bifrost} icon="dmn-diff/history-preview" />
          <EditorTitleText
            studio={bifrost}
      ...
      <EditorToolbar>
        <EditorToolbarLeft>
          <EditorToolbarButton
            studio={bifrost}
            className={viewMode === 'diff' ? 'dmn-diff__history-previewmode--active' : ''}
            icon="ph ph-git-diff"
            tooltip="Compare current and selected versions"
            command="dmn.diff.historyPreview.changeViewMode"
            commandArgs={[model, 'diff']}
          />
          <EditorToolbarButton
            studio={bifrost}
            className={viewMode === 'preview' ? 'dmn-diff__history-previewmode--active' : ''}
            icon="ph ph-eye"
        ...
        <EditorToolbarRight>
          <EditorToolbarButton
            ...
            command="dmn.diff.history.restoreFile"
            commandArgs={[editorDocument]}
          />
        </EditorToolbarRight>
      </EditorToolbar>
