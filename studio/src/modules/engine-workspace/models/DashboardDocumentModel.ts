...
export class DashboardDocumentModel extends EditorDocumentModel {
  ...
  private info: EngineInfoResponse | null = null;
  ...
  static async create(
    ...
    _fileLoader: any,
    studio: Bifrost,
  ): Promise<DashboardDocumentModel> {
    return new DashboardDocumentModel(uri, studio);
  }
  ...
  onEditorDocumentModelDidRegister(): void {
    ...
    this.autoRefresh = new EventDrivenRefresh({
      ...
      studio: this.studio,
      settingsKey: SETTINGS_KEYS.dashboardAutoRefresh,
      relevantEventTypes: [
        'EngineStarted',
        'EngineShutdown',
        'EngineOverloaded',
  ...
  getInfo(): EngineInfoResponse | null {
    return this.info;
  }

  getStats(): StatsResponse | null {
    return this.stats;
  }
  ...
  private refreshLabel(): void {
    ...
    this.updateLabel(`Dashboard: ${name}`);
  }
  ...
  async refresh(): Promise<void> {
    ...
    try {
      const [info, stats] = await Promise.all([this.client.engine.info(), this.client.engine.stats()]);

      let healthy: boolean;
      try {
        await this.client.engine.health();
        healthy = true;
      } catch (healthCheckError) {
        console.warn('[Dashboard] Engine health check failed:', healthCheckError);
        healthy = false;
      }
