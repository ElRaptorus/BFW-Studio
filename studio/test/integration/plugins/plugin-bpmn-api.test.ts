...
describe('plugin/bpmn-api', () => {
  describe('permission gating', () => {
    ...
    beforeAll(async () => {
      studioAgent = await createAndStartStudioAgentForPluginHost(
        { testName: 'bpmn-api-perm', testFile: __filename },
        { pluginsDirectory: PLUGINS_FIXTURE_DIR, waitUntilLoaded: ['bpmn-no-perm'] },
      );
    });
    ...
    it('denies bpmn.setOverlays to plugin without bpmn permission', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-no-perm.trySetOverlays');
    ...
    it('denies bpmn.getElements to plugin without bpmn permission', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-no-perm.tryGetElements');
      assert.ok(typeof result === 'string', 'Expected string error message');
      assert.ok(
        result.toLowerCase().includes('denied') || result.toLowerCase().includes('permission'),
        `Expected permission denial, got: ${result}`,
    ...
    it('denies bpmn.onElementSelected to plugin without bpmn permission', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-no-perm.trySubscribe');
      assert.ok(typeof result === 'string', 'Expected string error message');
      assert.ok(
        result.toLowerCase().includes('denied') || result.toLowerCase().includes('permission'),
        `Expected permission denial, got: ${result}`,
    ...
    it('denies bpmn.getXml to plugin without bpmn permission', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-no-perm.tryGetXml');
      assert.ok(typeof result === 'string', 'Expected string error message');
      assert.ok(
        result.toLowerCase().includes('denied') || result.toLowerCase().includes('permission'),
        `Expected permission denial, got: ${result}`,
  ...
  describe('overlay lifecycle', () => {
    ...
    beforeAll(async () => {
      studioAgent = await createAndStartStudioAgentForPluginHost(
        { testName: 'bpmn-api-overlays', testFile: __filename },
        { pluginsDirectory: PLUGINS_FIXTURE_DIR, waitUntilLoaded: ['bpmn-overlay-demo'] },
      );
    ...
    it('sets overlays on a BPMN element', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.setOverlays', [bpmnUri]);
      assert.strictEqual(result, 'ok', `Expected 'ok', got: ${result}`);

      const isSet = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.isOverlaysSet');
      assert.strictEqual(isSet, true);
    });

    it('clears all overlays for a URI', async () => {
      await studioAgent.executeCommand('plugin.bpmn-overlay-demo.setOverlays', [bpmnUri]);

      const result = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.clearOverlays', [bpmnUri]);
      assert.strictEqual(result, 'ok', `Expected 'ok', got: ${result}`);
    ...
    it('returns error when setting overlays on non-existent URI', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.setOverlaysOnMissingUri');
    ...
    it('returns error when subscribing to events on non-existent URI', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.subscribeOnMissingUri');
      assert.ok(typeof result === 'string', 'Expected string');
      assert.ok(result.startsWith('error:'), `Expected error, got: ${result}`);
    });
  });
  ...
  describe('overlay factory auto-render', () => {
    ...
    beforeAll(async () => {
      studioAgent = await createAndStartStudioAgentForPluginHost(
        { testName: 'bpmn-api-factory', testFile: __filename },
        { pluginsDirectory: PLUGINS_FIXTURE_DIR, waitUntilLoaded: ['bpmn-overlay-demo'] },
      );
      await studioAgent.openFixturesDirectoryAsSolution('test-solution-bpmn');
    });
    ...
    it('overlay factory is invoked automatically when BPMN file opens', async () => {
      ...
      await studioAgent.waitUntil(
        async () => {
          const count = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.getFactoryCallCount');
          return typeof count === 'number' && count > 0;
        },
        { timeout: 10_000, timeoutMsg: 'Overlay factory was never called after opening BPMN file' },
      );

      const callCount = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.getFactoryCallCount');
  ...
  describe('element queries', () => {
    ...
    beforeAll(async () => {
      studioAgent = await createAndStartStudioAgentForPluginHost(
        { testName: 'bpmn-api-queries', testFile: __filename },
        { pluginsDirectory: PLUGINS_FIXTURE_DIR, waitUntilLoaded: ['bpmn-overlay-demo'] },
      );
      await studioAgent.openFixturesDirectoryAsSolution('test-solution-bpmn');
      bpmnUri = await openBpmnFileAndGetUri(studioAgent, 'definition.bpmn');
    ...
    it('getElements returns non-empty array of element snapshots', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.getElements', [bpmnUri]);
    ...
    it('getElement returns detail snapshot for existing element', async () => {
      const elements = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.getElements', [bpmnUri]);
      assert.ok(Array.isArray(elements) && elements.length > 0);

      const targetId = elements[0].id;
      const detail = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.getElement', [bpmnUri, targetId]);
    ...
    it('getElement returns null for non-existent element', async () => {
      const detail = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.getElement', [
        bpmnUri,
    ...
    it('getXml returns valid XML string', async () => {
      const result = await studioAgent.executeCommand('plugin.bpmn-overlay-demo.getXml', [bpmnUri]);
      assert.strictEqual(result, 'ok', `Expected 'ok', got: ${result}`);
    });
  });
});
