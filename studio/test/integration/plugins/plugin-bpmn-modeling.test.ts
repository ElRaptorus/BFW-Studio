...
describe('plugin/bpmn-modeling', () => {
  describe('permission gating — bpmn.modelling required for modeling.*', () => {
    ...
    beforeAll(async () => {
      studioAgent = await createAndStartStudioAgentForPluginHost(
        { testName: 'bpmn-modeling-perm', testFile: __filename },
  ...
  describe('modeling operations', () => {
    ...
    beforeAll(async () => {
      studioAgent = await createAndStartStudioAgentForPluginHost(
        { testName: 'bpmn-modeling-ops', testFile: __filename },
        {
          pluginsDirectory: PLUGINS_FIXTURE_DIR,
          waitUntilCommandRegistered: ['plugin.bpmn-palette-demo.test.modeling.updateProperties'],
        },
      );
    ...
    it('updateProperties changes element name', async () => {
      ...
      const result = await studioAgent.executeCommand('plugin.bpmn-palette-demo.test.modeling.updateProperties', [
        bpmnUri,
        targetId,
    ...
    it('appendElement creates a new connected element', async () => {
      ...
      const result = await studioAgent.executeCommand('plugin.bpmn-palette-demo.test.modeling.appendElement', [
        bpmnUri,
        sourceId,
    ...
    it('moveElement changes element position', async () => {
      ...
      const result = await studioAgent.executeCommand('plugin.bpmn-palette-demo.test.modeling.moveElement', [
        bpmnUri,
        elementData.id,
        { x: 30, y: -20 },
      ]);
    ...
    it('removeElement removes the element from canvas', async () => {
      ...
      const appendResult = await studioAgent.executeCommand('plugin.bpmn-palette-demo.test.modeling.appendElement', [
        bpmnUri,
        sourceId,
      ...
      const removeResult = await studioAgent.executeCommand('plugin.bpmn-palette-demo.test.modeling.removeElement', [
        bpmnUri,
        toDeleteId,
      ]);
