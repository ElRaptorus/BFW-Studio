...
describe('plugin/dmn-modeling', { timeout: 120_000 }, () => {
  describe('permission gating — dmn.modelling required for modeling.*', () => {
    ...
    beforeAll(async () => {
      studioAgent = await createAndStartStudioAgentForPluginHost<StudioAgentDmnExtension>(
        { testName: 'dmn-modeling-perm', testFile: __filename },
        {
    ...
    it('denies modeling.updateProperties to plugin without dmn.modelling', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-perm-low.tryModelingUpdateProperties');
    ...
    it('denies modeling.removeElement to plugin without dmn.modelling', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-perm-low.tryModelingRemoveElement');
      assert.ok(typeof result === 'string', 'Expected string error message');
      assert.ok(
        result.toLowerCase().includes('denied') || result.toLowerCase().includes('permission'),
        `Expected permission denial, got: ${result}`,
    ...
    it('denies modeling.createElement to plugin without dmn.modelling', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-perm-low.tryModelingCreateElement');
      assert.ok(typeof result === 'string', 'Expected string error message');
      assert.ok(
        result.toLowerCase().includes('denied') || result.toLowerCase().includes('permission'),
        `Expected permission denial, got: ${result}`,
    ...
    it('denies modeling.appendElement to plugin without dmn.modelling', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-perm-low.tryModelingAppendElement');
      assert.ok(typeof result === 'string', 'Expected string error message');
      assert.ok(
        result.toLowerCase().includes('denied') || result.toLowerCase().includes('permission'),
        `Expected permission denial, got: ${result}`,
    ...
    it('denies modeling.createConnection to plugin without dmn.modelling', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-perm-low.tryModelingCreateConnection');
      assert.ok(typeof result === 'string', 'Expected string error message');
      assert.ok(
        result.toLowerCase().includes('denied') || result.toLowerCase().includes('permission'),
        `Expected permission denial, got: ${result}`,
    ...
    it('denies modeling.moveElement to plugin without dmn.modelling', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-perm-low.tryModelingMoveElement');
      assert.ok(typeof result === 'string', 'Expected string error message');
      assert.ok(
        result.toLowerCase().includes('denied') || result.toLowerCase().includes('permission'),
        `Expected permission denial, got: ${result}`,
  ...
  describe('modeling operations', () => {
    ...
    beforeAll(async () => {
      studioAgent = await createAndStartStudioAgentForPluginHost<StudioAgentDmnExtension>(
        { testName: 'dmn-modeling-ops', testFile: __filename },
        {
          pluginsDirectory: PLUGINS_FIXTURE_DIR,
          studioAgentClass: StudioAgentDmnExtension,
        },
    ...
    it('updateProperties changes element name', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.modeling.updateProperties', [
        dmnUri,
        'Decision_Discount',
      ...
      const detail = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.getElement', [
        dmnUri,
        'Decision_Discount',
      ]);
    ...
    it('createElement places a new element at an absolute position', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.modeling.createElement', [
        dmnUri,
        {
          type: 'dmn:InputData',
    ...
    it('appendElement creates a new element connected to the source', async () => {
      const result = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.modeling.appendElement', [
        dmnUri,
        'InputData_Age',
    ...
    it('moveElement changes element position', async () => {
      const before = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.getElement', [
        dmnUri,
        'InputData_Age',
      ]);
      assert.ok(before != null, 'Expected InputData_Age to exist');

      const result = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.modeling.moveElement', [
        dmnUri,
        'InputData_Age',
        { x: 30, y: -20 },
      ]);
      ...
      const after = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.getElement', [
        dmnUri,
        'InputData_Age',
      ]);
    ...
    it('removeElement removes the element from the DRD', async () => {
      const createResult = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.modeling.createElement', [
        dmnUri,
      ...
      const removeResult = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.modeling.removeElement', [
        dmnUri,
        toDeleteId,
      ]);
      assert.strictEqual(removeResult, 'ok', `Expected 'ok', got: ${removeResult}`);

      const detail = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.getElement', [dmnUri, toDeleteId]);
    ...
    it('createConnection links two existing elements', async () => {
      const inputResult = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.modeling.createElement', [
        dmnUri,
        { type: 'dmn:InputData', name: 'Connection Source', position: { x: 600, y: 200 } },
      ]);
      assert.ok(inputResult != null && 'elementId' in inputResult);

      const decisionResult = await studioAgent.executeCommand('plugin.dmn-palette-demo.test.modeling.createElement', [
        dmnUri,
        { type: 'dmn:Decision', name: 'Connection Target', position: { x: 600, y: 320 } },
      ]);
      assert.ok(decisionResult != null && 'elementId' in decisionResult);

      const connectionResult = await studioAgent.executeCommand(
        'plugin.dmn-palette-demo.test.modeling.createConnection',
        [dmnUri, inputResult.elementId, decisionResult.elementId],
      );
      assert.ok(
